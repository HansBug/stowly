...
await step('settings + solve', async () => {
  ...
  await shot('solving')
  if (!(await page.getByTestId('solve-progress').isVisible())) problems.push('[ui] progress panel not shown while solving')
  const live = await page.getByTestId('progress-live').innerText().catch(() => '')
