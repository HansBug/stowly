...
- **A time budget you do not have to guess.** By default the solve time follows the cargo: packingsolver3d's `recommend_time_budget` (one formula per upstream algorithm, fitted on 3158 recorded runs) sets the time limit and a stall-stop rule, and the solve usually ends well before the limit. A *Set manually* switch opens the three values pre-filled with the recommendation; a *Preference* choice (faster / balanced / thorough) moves the dial; the first-solution time of every solve calibrates a machine-speed factor kept in the app.
...
How long to wait is decided per project. Upstream's anytime algorithms run until their time limit, so the backend asks packingsolver3d's `recommend_time_budget` for a `time_limit` plus `stop_when_unimproved_for` / `_after` (the *Automatic* mode; `POST /api/recommend` returns the same numbers for display). The estimate knows which upstream algorithm the instance will take (`algorithm_path`) and predicts its first-solution latency and the time worth waiting after it; the *Preference* choice is the estimator's `alpha` (2 / 4 / 8; default 4 for `box`, 8 for `boxstacks`). Every finished solve compares the predicted and the observed first-solution time and updates a machine-speed factor stored in `localStorage['stowly.calibration']`, so the second run on a slow or fast machine is already scaled. In *Set manually* mode the three values are the user's, pre-filled with the recommendation. While a job runs, `GET /api/jobs/{id}` also carries the budget and the improvement events reported by the solver, which the progress panel shows.
