...
- **Three objectives**: pack everything into as few containers as possible, fill one container with the most valuable subset (knapsack, value defaults to volume), or choose the cheapest mix of container sizes.
- **Two solvers with different physics.** `box` is free three-dimensional packing: any rotation, no support constraint, so a solution may contain floating pieces and its output order is the search order, not a loading order; the viewer says so and counts the floating pieces. `boxstacks` keeps cargo upright ("any" rotation is treated as upright) and builds stacks of identical cargo that stand on the floor; its output is stack by stack from the floor up and can be used as a loading order.
- **Stacking rules for `boxstacks`**: most pieces per stack, heaviest load allowed on top of a piece, nesting depth, unloading groups with an unloading constraint (door at the x-max or y-max end, group 0 unloaded first) and a floor-load limit per container. The columns appear when `boxstacks` is selected; the `box` solver has no stacking model, and the interface says so instead of silently ignoring the values.
- **Open sides per container**: doors and open tops are part of the container definition (presets carry them: container and truck doors at the far end, pallets open all around) and are drawn amber in the 3D view; the floor is a dark grid plate with a gravity arrow and floating pieces are outlined in red, so the loading direction and what rests on what are visible from any angle.
- **A time budget you do not have to guess.** By default the solve time follows the cargo: packingsolver3d's `recommend_time_budget` (one formula per upstream algorithm, fitted on 3158 recorded runs) sets the time limit and a stall-stop rule whose patience follows the time of the last improvement (packingsolver3d 0.0.4's `stop_when_unimproved_ratio`), and the solve usually ends well before the limit. A *Set manually* switch opens the four values (time limit, fixed patience, earliest stop, patience ratio) pre-filled with the recommendation; a *Preference* choice (faster / balanced / thorough) moves the dial; the first-solution time of every solve calibrates a machine-speed factor kept in the app.
- **Live progress**: a time-driven bar, the current best (items, containers, value, time since the last improvement), the upstream algorithm's own improvement log, and how the run ended (stall stop, time limit, proven optimal).
...
3. Open **Solver**: `boxstacks`, objective *Knapsack: highest value*, solve time *Automatic (recommended)* — the line below it says how long the first solution and the limit are expected to take. The line under the settings compares total cargo volume with container capacity.
...
Two solvers are offered because they model different things. `box` (upstream `packingsolver::box`) is a pure geometric packing: items may be rotated in all six ways and nothing requires a piece to rest on another, so the result can float and the order of placements is the tree search's insertion order. `boxstacks` (upstream `packingsolver::boxstacks`) packs upright stacks of identical cargo standing on the floor with stacking limits, weight above, floor load, axle rules and unloading constraints; Stowly maps "any" rotation to upright there because upstream only places XYZ/YXZ and its algorithm does not cope with tipping rotations, gives every cargo type its own stack id, and forwards the stacking columns. `boxstacks` is dense and reliable when everything fits (bin packing) and for single-type knapsacks; for mixed cargo that does not fit, upstream's sequential heuristic may return nothing within the time limit, a limitation of the upstream algorithm that Stowly reports as *no solution*.

How long to wait is decided per project. Upstream's anytime algorithms run until their time limit, so the backend asks packingsolver3d's `recommend_time_budget` for a `time_limit` plus `stop_when_unimproved_for` / `_after` / `_ratio` (the *Automatic* mode; `POST /api/recommend` returns the same numbers for display). The estimate knows which upstream algorithm the instance will take (`algorithm_path`) and predicts its first-solution latency and the time worth waiting after it; the *Preference* choice is the estimator's `alpha` (2 / 4 / 8; default 4 for `box`, 8 for `boxstacks`). Every finished solve compares the predicted and the observed first-solution time and updates a machine-speed factor stored in `localStorage['stowly.calibration']`, so the second run on a slow or fast machine is already scaled. Paths that report nothing before their first full pass (multi-bin `boxstacks` knapsack / variable-sized bin packing, upstream `SVC`) are budgeted for that pass: the speed factor only ever lengthens their limit, an automatic run that still ends without a solution is given one more attempt with twice the limit (the progress panel says so), and such runs do not calibrate the speed. In *Set manually* mode the four values are the user's, pre-filled with the recommendation. While a job runs, `GET /api/jobs/{id}` also carries the budget and the improvement events reported by the solver, which the progress panel shows.
