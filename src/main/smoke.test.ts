...
describe('smokeTest', () => {
  ...
  it('flags HTTP errors, wrong placements, a slow job and a blank renderer', async () => {
    ...
    let report = await smokeTest({ info, startupError: null, log: [], location, renderer: async () => false, fetchImpl: fakeFetch(routes), sleep })
    expect(report.errors).toEqual(expect.arrayContaining([expect.stringContaining('presets: /api/presets -> 401'), 'solve placed 9 of 10 items', 'renderer did not show the app with presets loaded']))
