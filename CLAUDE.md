...
| Linux sandbox | Hosts that restrict unprivileged user namespaces (Ubuntu 24.04+): the portable tar.gz starts through `build/linux-launcher.sh`, which falls back to `--no-sandbox` when neither user namespaces nor a setuid `chrome-sandbox` can work (electron-builder's AppImage AppRun already does the same with `unshare -Ur true`, so the launcher only passes its flag through there); the `.deb` relies on electron-builder's AppArmor profile | Chromium's sandbox helper | `make run` detects `kernel.apparmor_restrict_unprivileged_userns`; Build Desktop runs the extracted tar.gz inside the bare `ubuntu:22.04` container without `--no-sandbox` (Docker's seccomp blocks user namespaces) and expects the launcher's notice; the Ubuntu 24.04 case itself was verified by hand, the runners' kernels do not enforce the sysctl |
| Windows | 10 / 11, x64 and arm64 | Electron 44 dropped 7–8.1; arm64 is built natively on `windows-11-arm` (python-build-standalone `aarch64-pc-windows-msvc`, packingsolver3d `win_arm64` wheels) | Build Desktop smoke of the extracted portable zip and of a silent install (NSIS `/S /D=` on x64, Inno Setup `/VERYSILENT /DIR=` on arm64) with `PATH` reduced to `System32` |
...
- `BinSpec { id, name, x, y, z, copies, cost?, maxWeight?, maxStackDensity?, openSides: Side[] }` with `Side = 'x-min' | 'x-max' | 'y-min' | 'y-max' | 'top'` (x is the length axis, `x-max` the door end; presets carry their sides, `parseProject` defaults old files to `['x-max']`); `ItemSpec { id, name, x, y, z, copies, weight?, profit?, rotations: 'all' | 'upright' | 'fixed', color?, maxStack?, maxWeightAbove?, nestingHeight?, group }`; `Settings { solver: 'box' | 'boxstacks', objective: 'bin-packing' | 'knapsack' | 'variable-sized-bin-packing', timeLimit, optimizationMode, unloadingConstraint }`.
- `solver.build_instance` maps rotations to packingsolver3d tokens (`all` → six rotations, `upright` → `XYZ, YXZ`, `fixed` → `XYZ`). For `boxstacks` it maps `all` to upright as well: upstream boxstacks only places XYZ/YXZ, packingsolver3d rejects tipping-only sets, and a mixed set makes upstream's SOR algorithm return nothing (measured: one carton type × 30 on a EUR pallet, six rotations → no solution in 10 s; upright → optimal 16 in 0.05 s). It gives every item type its own `stackability_id` so only identical types stack (upstream buckets types by `(group_id, stackability_id)` and refuses mixed footprints) and forwards the boxstacks-only fields: `maxStack` → `maximum_stackability`, `maxWeightAbove` → `maximum_weight_above`, `nestingHeight` → `nesting_height` (upstream lowers the item above by it: `z_start -= nesting_height`), `group` → `group_id`, bin `maxStackDensity` (kg/m²) → `maximum_stack_density` (kg/mm², divided by 1e6), `settings.unloadingConstraint` → `Instance.unloading_constraint`. For `box` none of these is sent; packingsolver3d's `box.validate` would refuse them, and the interface hides the columns with a note instead. Known upstream limitation, deliberately not reported upstream yet: `boxstacks` single-bin knapsack runs the SOR heuristic first and only starts the tree search when SOR violated axle constraints (`src/boxstacks/optimize.cpp`), so mixed-type knapsacks that do not fully fit can end with no solution (two carton types × 16 on a pallet: nothing in 30 s; × 12: optimal instantly).
...
- `three/scene.ts` owns WebGL: renderer, camera, OrbitControls, raycasting for hover/select, visibility for the order slider, and the container walls (`addWalls`: closed faces light grey, open sides amber with an outline; solver x/y/z map to three.js x/z/y). The floor is the one opaque face (`addFloor`: dark plate, 0.5 m grid, a gravity arrow beside the origin corner) and placements listed in `setBin`'s `highlight` set (the floating ones, computed by `floatingPlacements`) get a red outline, so "down", "on the floor" and "hanging in the air" can be read from any camera angle. Geometry is computed in `three/boxes.ts`, which is pure and tested directly. `Viewer3D` catches a failing `SceneController` constructor (no WebGL context: GPU-less virtual machines, remote desktops) and shows a notice instead of letting the whole React tree crash; the macOS x64 CI runner is exactly such a machine, and the smoke test's renderer check depends on this.
...
|- scripts/after-pack.cjs          # electron-builder afterPack hook, Linux only: executable -> stowly-bin, launcher installed as stowly
|- build/linux-launcher.sh         # the `stowly` launcher of the Linux packages: sandbox check, --no-sandbox fallback with a notice
|- build/stowly-win-arm64.iss      # Inno Setup script for the Windows arm64 installer (NSIS does not work on Windows on ARM)
...
- `electron-builder` packages `out/` plus `resources/python` (as `resources/python` inside the app; `extraResources`). On Linux `scripts/after-pack.cjs` (`afterPack`) renames the executable to `stowly-bin` and installs `build/linux-launcher.sh` as `stowly`, so the portable archive, the AppImage's AppRun, the `.desktop` entry and `/usr/bin/stowly` all go through it: the script execs the binary as is when `chrome-sandbox` is setuid root or user namespaces are usable, and otherwise adds `--no-sandbox` with a one-line notice on stderr. The reason is Ubuntu 24.04+ (`kernel.apparmor_restrict_unprivileged_userns=1`): an unconfined process may still create a user namespace, but AppArmor moves it into the `unprivileged_userns` profile, which strips the capabilities Chromium's namespace sandbox needs, so Chromium falls back to the SUID helper, which an archive extracted by a user cannot have, and aborts with "The SUID sandbox helper binary was found, but is not configured correctly" (the downloaded portable did exactly this before the launcher). The launcher therefore treats "sysctl is 1 and my own label is `unconfined`" as restricted, and additionally `unshare -Ur true` failing (containers with seccomp, `unprivileged_userns_clone=0`, `max_user_namespaces=0`; the same probe electron-builder's AppImage AppRun uses, which is why the AppImage already added `--no-sandbox` by itself and the launcher just passes it through). Plain `unshare -U true` succeeds on such hosts and proves nothing. The `.deb` keeps its sandbox without the fallback: electron-builder's postinst installs an AppArmor profile attached to `/opt/Stowly/stowly`, a profile attached to a script path applies when the script is executed, and being `flags=(unconfined)` its label stays on the binary the script execs (verified on Ubuntu 24.04 with a throwaway profile), so the launcher sees a non-`unconfined` label and execs `stowly-bin` directly. Every platform ships a **portable** archive (extract and run) and an **installer** per architecture, named `${productName}-${version}-<os>-<arch>-{portable|installer}.<ext>` through `artifactName` patterns: Linux `tar.gz` / `deb` (+ the single-file AppImage; `toolsets.appimage: "1.0.3"` selects electron-builder's static AppImage runtime because the legacy arm64 runtime links an unversioned `libz.so` and fails to load on stock distributions), Windows `zip` / `nsis` on x64 and `zip` / Inno Setup on arm64 (`build/stowly-win-arm64.iss`, compiled by the workflow with ISCC from the unpacked directory; per-user install like the NSIS one), macOS `zip` / `dmg`. **Do not retry NSIS for Windows arm64 without new evidence**: on the `windows-11-arm` runner the electron-builder installer detects the architecture correctly (`packageArch=ARM64`, `IsNativeARM64=1`) yet the legacy NSIS 3.0.4.1 bundle leaves only the uninstaller and registry entry behind (7z payload extraction produces nothing), the `toolsets.nsis: 1.2.1` bundle (NSIS 3.12) extracts only `resources/` and `resources.pak`, and both `nsis.useZip` variants hang in silent mode; the smoke test caught all of it. The config lists no `arch`: an arch list in the config wins over the CLI, so the workflow's `--x64` / `--arm64` would be ignored and each runner would also package the other architecture with the wrong CPython inside (this happened before the flags were honoured). electron-builder spells the architecture per format (`amd64` in deb, `x86_64` in AppImage); the workflow renames those to `x64` right after packaging so every file name uses `x64` / `arm64`. The `.deb` declares Electron's full runtime dependency list explicitly (`deb.depends`): electron-builder's default omits `libgbm1` and `libasound2`, and a minimal Ubuntu 20.04 then fails with `libgbm.so.1: cannot open shared object file`; the container test caught it.
...
- Chromium aborts with "SUID sandbox helper binary was found, but is not configured correctly": the host restricts unprivileged user namespaces. Packaged builds go through `build/linux-launcher.sh`, which handles it; in development pass `--noSandbox` (`make run` does), and if the launcher was bypassed run `stowly-bin --no-sandbox`.
