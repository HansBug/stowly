...
- `three/scene.ts` owns WebGL: renderer, camera, OrbitControls, raycasting for hover/select, visibility for the loading-order slider. Geometry is computed in `three/boxes.ts`, which is pure and tested directly.
...
- `electron-builder` packages `out/` plus `resources/python` (as `resources/python` inside the app; `extraResources`). Targets: Linux `deb` + `AppImage` (x64), Windows `nsis` (x64), macOS `dmg`. The workflow passes `--mac --arm64` or `--mac --x64` so each macOS runner packages only its own architecture (the bundled interpreter is architecture-specific; a cross-packaged DMG would carry the wrong one).
