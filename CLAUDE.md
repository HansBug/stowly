...
|---|---|---|---|
| Linux runtime | x86_64, glibc ≥ 2.31 (Ubuntu 20.04+) | Electron 44's floor; python-build-standalone `install_only` builds need only glibc 2.17 | Build Desktop installs the `.deb` in `ubuntu:20.04` and runs `--smoke`; the AppImage runs in `ubuntu:22.04` |
...
| Windows | 10 / 11 x64 | Electron 44 dropped 7–8.1 | Build Desktop smoke on `windows-2022` with `PATH` reduced to `System32` |
| macOS | 12+, arm64 and x64, one architecture per build | Electron 44, python-build-standalone 3.12 | Build Desktop smoke on `macos-14` (arm64) and `macos-15-intel` (x64) with `env -i` |
| Bundled Python | CPython 3.12 from python-build-standalone, pinned in `scripts/prepare-python.mjs` | packingsolver3d wheels exist for 3.7–3.14 on all four targets | The smoke report names the interpreter path; CI asserts it is inside the package's resources |
...
- **Startup handshake.** The backend prints exactly one `READY` JSON line; everything else it prints (uvicorn logs, tracebacks) is kept in a 500-line ring buffer and shown in the interface if the start fails. `BackendProcess.start` rejects on exit, spawn error or a 30 s timeout. `PythonLocation.args` lets tests run a fake interpreter through `node`.
...
- `electron-builder` packages `out/` plus `resources/python` (as `resources/python` inside the app; `extraResources`). Targets: Linux `deb` + `AppImage` (x64), Windows `nsis` (x64), macOS `dmg`. The `.deb` declares Electron's full runtime dependency list explicitly (`deb.depends` in `electron-builder.yml`): electron-builder's default omits `libgbm1` and `libasound2`, and a minimal Ubuntu 20.04 then fails with `libgbm.so.1: cannot open shared object file`; the container test caught it. The workflow passes `--mac --arm64` or `--mac --x64` so each macOS runner packages only its own architecture (the bundled interpreter is architecture-specific; a cross-packaged DMG would carry the wrong one).
- **Build Desktop** (`.github/workflows/build.yaml`) runs on pushes to `main`, `ci/**` branches, tags `v*`, pull requests and by hand: package on four runners, smoke-test each package in a clean environment, upload `stowly-<target>` artifacts (installers + `smoke-*.json`), and on a `v*` tag create the GitHub release with the same files and generated notes.
