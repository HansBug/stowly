...
- **不用猜的求解时间。**默认由货物决定：packingsolver3d 的 `recommend_time_budget`（按上游算法路径各一个公式，在 3158 次记录运行上拟合）给出时间上限和停滞即停规则（等待时长跟着上次改进的时刻走，即 packingsolver3d 0.0.4 的 `stop_when_unimproved_ratio`），求解通常远早于上限结束。点"手动设置"会展开四个已填好推荐值的字段（时间上限、固定耐心、最早可停时刻、耐心倍数）；"偏好"（更快 / 均衡 / 更充分）拨动权衡；每次求解的首解时间会校准一个保存在应用里的机器速度系数。
...
工具栏随时可以切换长度单位（mm / cm / m / in），数值自动换算，求解器收到的永远是毫米。语言用 中文 / EN / 日本語 开关切换。
