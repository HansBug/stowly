...
- **不用猜的求解时间。**默认由货物决定：packingsolver3d 的 `recommend_time_budget`（按上游算法路径各一个公式，在 3158 次记录运行上拟合）给出时间上限和停滞即停规则，求解通常远早于上限结束。点"手动设置"会展开三个已填好推荐值的字段；"偏好"（更快 / 均衡 / 更充分）拨动权衡；每次求解的首解时间会校准一个保存在应用里的机器速度系数。
- **实时进度**：按时间推进的进度条、当前最优（件数、容器数、价值、距上次改进的秒数）、上游算法自己的改进记录，以及本次是如何结束的（停滞即停 / 到达上限 / 已证明最优）。
...
| 平台 | 绿色版（解压即用） | 安装版 | 说明 |
|---|---|---|---|
| Linux x64 / arm64，glibc ≥ 2.31（Ubuntu 20.04+、Debian 11+、RHEL 9+） | `…-linux-<arch>-portable.tar.gz` → `./stowly` | `…-linux-<arch>-installer.deb`（装到 `/opt/Stowly`），或单文件 `…-linux-<arch>.AppImage` | `stowly` 是一个小启动脚本，会先检查 Chromium 沙箱能否启动。在限制非特权 user namespace 的发行版（Ubuntu 24.04 及以后）上，绿色版会自动退回 `--no-sandbox` 运行并在 stderr 说明（AppImage 自带的 AppRun 也做同样的检查）；想保留沙箱，在解压目录执行一次 `sudo chown root:root chrome-sandbox && sudo chmod 4755 chrome-sandbox` 即可。`.deb` 会安装 AppArmor 配置文件，无需任何操作。 |
...
3. 打开**求解设置**：`boxstacks`，目标"背包：价值最大"，求解时间保持"自动（推荐）"，下面那行会写出预计的首解时间和上限。设置下方那行把货物总体积和容器总容积做了对比。
...
等多久由项目决定。上游的 anytime 算法会一直跑到时限，所以后端向 packingsolver3d 的 `recommend_time_budget` 要一个 `time_limit` 加 `stop_when_unimproved_for` / `_after`（"自动"模式；`POST /api/recommend` 返回同样的数字供界面显示）。估算知道该实例会走上游哪条算法（`algorithm_path`），预测它的首解延迟和之后值得再等的时间；"偏好"就是估算器的 `alpha`（2 / 4 / 8；`box` 默认 4，`boxstacks` 默认 8）。每次求解结束后用预测与实测的首解时间更新一个存在 `localStorage['stowly.calibration']` 里的机器速度系数，慢机器或快机器上第二次求解就已经按比例缩放。"手动设置"模式下三个值由用户决定，初值是推荐值。任务运行期间 `GET /api/jobs/{id}` 同时带着预算和求解器报告的改进事件，进度面板据此显示。
